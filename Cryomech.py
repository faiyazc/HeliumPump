...
#gTxtFile = Text(gWindow, height=1, width=40)
gAlert = scrolledtext.ScrolledText(gWindow)

def Start_Clicked():
    gTxtFeedback.insert(INSERT, 'Start button clicked\r\n')
    HOST = gTxtIP.get('1.0', END) #'169.254.107.38'    # The compressor's IP address
    HOST = HOST.strip()
...
def main():
    ...
    lblIP = Label(gWindow, text="IP Address: (Default:169.254.107.38)")
    lblIP.grid(column=0, row=0)

    gTxtIP.text = '169.254.107.38'
    gTxtIP.grid(column=1, row=0)
    
    #lblFile = Label(gWindow, text="File Name: ")
    #lblTxt.grid(column=0, row = 1)
    
    #gTxtFile.text = 'logFile' + datetime.now().strftime("%H:%M:%S")
    #gTxtFile.grid(column=1, row = 1)
